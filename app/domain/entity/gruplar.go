...
var GruplarTableName string = "sacrifice_gruplar"
...
type SacrificeGruplar struct {
	...
	GrupAdi            string    `gorm:"type:varchar(255); null;" json:"grup_adi"`
	...
	Kurban    []SacrificeKurbanlar `gorm:"foreignKey:GrupID;references:ID"`
}

//BeforeSave init
func (gk *SacrificeGruplar) BeforeSave() {
	gk.Aciklama = html.EscapeString(strings.TrimSpace(gk.Aciklama))
}

//Prepare init
func (gk *SacrificeGruplar) Prepare() {
	gk.CreatedAt = time.Now()
	gk.UpdatedAt = time.Now()
}

//TableName override
func (gk *SacrificeGruplar) TableName() string {
	return "sacrifice_gruplar"
}

//Validate fluent validation
func (gk *SacrificeGruplar) Validate() map[string]string {
	var (
