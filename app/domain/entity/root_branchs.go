...
//TODO: ilerde sub RootBranch gelebilir
...
//RootBranchSave strcut
type RootBranchSaveDTO struct {
	ID               uint64 `gorm:"primary_key;auto_increment" json:"id"`
...
//BeforeSave init
func (f *RootBranch) BeforeSave() {
	f.Name = html.EscapeString(strings.TrimSpace(f.Name))
}

//Prepare init
func (f *RootBranch) Prepare() {
	f.Name = html.EscapeString(strings.TrimSpace(f.Name))
...
//TableName override
func (gk *RootBranch) TableName() string {
	return "root_branchs"
}

//ValidateV1 old version
func (f *RootBranch) ValidateV1() map[string]string {
...
func (f *RootBranch) Validate() map[string]string {
	var (
