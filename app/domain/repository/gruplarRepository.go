...
//Save data
func (r *GruplarRepo) Save(post *entity.SacrificeGruplar) (*entity.SacrificeGruplar, map[string]string) {
...
func (r *GruplarRepo) GetByAllRelations() ([]dto.GruplarExcelandIndex, error) {
	...
	err := r.db.Debug().Table("sacrifice_gruplar").Where("  durum <> 1  ").Order("kesim_sira_no asc").Find(&gruplarList).Error
	if err != nil {
		return nil, errors.New("database error, please try again")
	}
	if gorm.IsRecordNotFoundError(err) {
...
func (r *GruplarRepo) GetAllFindDurum(durum int) ([]entity.SacrificeGruplar, error) {
	var gruplarList []entity.SacrificeGruplar
...
func (r *GruplarRepo) GetAllFindDurumAndAgirlikTipi(durum int, agirlikTipi int) ([]entity.SacrificeGruplar, error) {
...
func (r *GruplarRepo) GetAllP(postsPerPage int, offset int) ([]entity.SacrificeGruplar, error) {
...
func (r *GruplarRepo) GrupToplamOdemeler(GrupID uint64) float64 {
	var result float64
	row := r.db.Debug().Table(entity.KurbanTableName).Select("sum(bakiye) as total").Where("grup_id = ? AND borc_durum <> 3 AND  borc_durum <> 7 AND durum <> 8 AND durum <> 7  AND durum <> 8", GrupID).Row()
	row.Scan(&result)
	return result
}
...
func (r *GruplarRepo) ToplamOdemeler() float64 {
	var result float64
	row := r.db.Debug().Table(entity.KurbanTableName).Select("sum(bakiye) as total").Where("borc_durum <> 3 AND  borc_durum <> 7 AND durum <> 8 AND durum <> 7  AND durum <> 8").Row()
	row.Scan(&result)
	return result
}

//ToplamOdemeler kurban id ye gore odenen miktar toplamı
func (r *GruplarRepo) KalanBorclar() float64 {
	var result float64
	row := r.db.Debug().Table(entity.KurbanTableName).Select("sum(alacak) as total").Where("  borc_durum <> 3 ").Row()
	row.Scan(&result)
	return result
}
