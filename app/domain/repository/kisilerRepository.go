...
func (r *KisilerRepo) GetKisiAciklama(id uint64, aciklamaOut *string) {
	var aciklama string
	row := r.db.Debug().Table(entity.KisilerTableName).Select("aciklama").Where("id =?  ", id).Row()
	row.Scan(&aciklama)
...
func (r *KisilerRepo) SetAciklama(kisiID uint64, value string) {
	r.db.Debug().Table(entity.KisilerTableName).Where(" id = ?", kisiID).Update("aciklama", value)
}
