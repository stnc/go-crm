...
var OdemelerBorcDurumTaksitSilindi string = stnccollection.IntToString(entity.OdemelerBorcDurumTaksitSilindi)
var OdemelerBorcDurumTaksitSilindiBildirim string = stnccollection.IntToString(entity.OdemelerBorcDurumTaksitSilindiBildirim)
...
//Save data
func (r *OdemelerRepo) Save(odemeler *entity.SacrificeOdemeler) (*entity.SacrificeOdemeler, map[string]string) {
...
func (r *OdemelerRepo) SaveBatch(odemeler []entity.SacrificeOdemeler) ([]entity.SacrificeOdemeler, map[string]string) {
	dbErr := map[string]string{}
	var err error
	err = r.db.Debug().Create(&odemeler).Error
	if err != nil {
...
//Update upate data
func (r *OdemelerRepo) Update(odemeler *entity.SacrificeOdemeler) (*entity.SacrificeOdemeler, map[string]string) {
	dbErr := map[string]string{}
	var err error
...
//Update upate data
func (r *OdemelerRepo) SetUpdateHesaplar(odemeler *dto.SacrificeOdemeler) (*dto.SacrificeOdemeler, map[string]string) {
	dbErr := map[string]string{}
	var err error
	err = r.db.Debug().Save(&odemeler).Error
	if err != nil {
...
func (r *OdemelerRepo) Delete(id uint64) error {
	var odemeler entity.SacrificeOdemeler
	var err error
...
//GetByID get data
func (r *OdemelerRepo) GetByID(id uint64) (*entity.SacrificeOdemeler, error) {
	var odemeler entity.SacrificeOdemeler
	var err error
...
//GetAll all data
func (r *OdemelerRepo) GetAll() ([]entity.SacrificeOdemeler, error) {
	var odemelers []entity.SacrificeOdemeler
	var err error
...
//TODO: get değeri ekle başına-- ayrıca bunun daha anlamlı olması lazım sql dekı last ıfadaesı falan geciyor
/* kullanım örneği
var kalanKurbanFiyati float64
if posts, err := odemeOptions.SonOdemeTablosu(kurbanIDUnit); err == nil {
	kalanKurbanFiyati = posts.Alacak
	fmt.Printf("%+v\n", posts)
}
*/
//SonOdemeTablosu get data last price
func (r *OdemelerRepo) SonOdemeTablosu(kurbanID uint64) (*dto.OdemelerSonFiyat, error) {
	var data dto.OdemelerSonFiyat
	var err error
	err = r.db.Debug().Table(entity.OdemelerTableName).Where("kurban_id = ?", kurbanID).Last(&data).Error
...
func (r *OdemelerRepo) OdemelerToplami(kurbanID uint64) float64 {
	var result float64
	row := r.db.Debug().Table(entity.OdemelerTableName).Select("sum(verilen_ucret) as total").Where("kurban_id = ? AND borc_durum <> "+odemelerBorcDurumKasaBorcluDurumda+" AND borc_durum <> "+ilkEklenenFiyatDegilse+" AND borc_durum <> "+OdemelerBorcDurumTaksitSilindi+" AND borc_durum <> "+OdemelerBorcDurumTaksitSilindiBildirim, kurbanID).Row()
	row.Scan(&result)
	return result
}
...
func (r *OdemelerRepo) OdemelerTopl2amiOLDeskiSistemeGoreYenideBakiyeEklendi(kurbanID uint64) float64 {
	...
	*/
	...
	row := r.db.Debug().Table(entity.OdemelerTableName).Select("sum(verilen_ucret) as total").Where("kurban_id = ? AND borc_durum <> "+odemelerBorcDurumKasaBorcluDurumda+" AND borc_durum <> "+ilkEklenenFiyatDegilse+"", kurbanID).Row()
	row.Scan(&result)
	/*
...
func (r *OdemelerRepo) KurbanSonKalanUcret(kurbanID uint64) (result float64) {
	row := r.db.Debug().Table(entity.OdemelerTableName).Select("alacak").Where("kurban_id = ?  AND borc_durum <> "+odemelerBorcDurumKasaBorcluDurumda+" ", kurbanID).Order("id desc").Row()
	row.Scan(&result)
	return result
}
...
//GetByHirePurchase taksit tutatını verr
func (r *OdemelerRepo) GetByHirePurchase(odemeID uint64, hirePurchase *float64) {
	var hirePurchaseData float64
	row := r.db.Debug().Table(entity.OdemelerTableName).Select("verilen_ucret").Where("id=  ? ", odemeID).Order("id asc").Row()
	row.Scan(&hirePurchaseData)
	*hirePurchase = hirePurchaseData
}

//OdemelerRepo upate data
func (r *OdemelerRepo) SetOdemelerBorcDurum(id uint64, status int) {
	r.db.Debug().Table(entity.OdemelerTableName).Where("id = ?", id).Update("borc_durum", status)
}
