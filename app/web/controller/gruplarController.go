...
func GrupDataGenerate(access *Gruplar) []dto.GruplarExcelandIndex {
	...
	for num, v := range veriler {
		...
		veriler[num].ToplamOdemeler = access.GruplarApp.GrupToplamOdemeler(v.ID)
		veriler[num].KalanBorclar = access.GruplarApp.GrupKalanBorclar(v.ID)
...
func (access *Gruplar) Index(c *gin.Context) {
	...
	viewData := pongo2.Context{
		...
		"flashMsg":       flashMsg,
		"msg":            msg,
		"toplamOdemeler": access.GruplarApp.ToplamOdemeler(),
		"toplamBorclar":  access.GruplarApp.KalanBorclar(),
	}
...
func (access *Gruplar) HayvanAtamasiStore(c *gin.Context) {
	...
	var dataGruplar entity.SacrificeGruplar
	...
	for no, _ := range kurbanlarData {
		...
		if odemelerToplami > kisiBasiDusenFiyat {
			...
			odemeEkle := entity.SacrificeOdemeler{}
			...
			// odemeEkle.VerildigiTarih = time.Now()////selBAK
		...
		if odemelerToplami < kisiBasiDusenFiyat {
			...
			odemeEkle.Durum = entity.OdemelerDurumGrupOlusmusKesimlikHayvaniVar
			odemeEkle.BorcDurum = entity.OdemelerBorcDurumHayvanEklendi
			odemeEkle.UserID = stncsession.GetUserID2(c)
			// odemeEkle.VerildigiTarih = time.Now()//selBAK
...
func (access *Gruplar) DegistirStore(c *gin.Context) {
	...
	var updateOdemeler = dto.SacrificeOdemeler{}
...
func (access *Gruplar) Excel(c *gin.Context) {
	...
	for num, v := range veriler {
		i++
		f.MergeCell("Sheet1", "A"+stnccollection.IntToString(i), "H"+stnccollection.IntToString(i))

		if v.AgirlikTipi == 1 {
			f.SetCellStyle("Sheet1", "A"+stnccollection.IntToString(i), "H"+stnccollection.IntToString(i), styleKucuk)
		} else if v.AgirlikTipi == 2 {
			f.SetCellStyle("Sheet1", "A"+stnccollection.IntToString(i), "H"+stnccollection.IntToString(i), styleOrta)
		} else if v.AgirlikTipi == 3 {
			f.SetCellStyle("Sheet1", "A"+stnccollection.IntToString(i), "H"+stnccollection.IntToString(i), styleBuyuk)

		}

		f.SetRowHeight("Sheet1", i, 20)

		kesimSiraNoText := stnccollection.IntToString(v.KesimSiraNo) + " - Sira No " + " / "
		agirlikTipiText := "Ağırlık: " + veriler[num].GrupIsoTopeTRname + " / "
		grupAdiText := "Grup: " + v.GrupAdi + " / "
		...
		toplamAgirlikText := " Ağırlık: " + stnccollection.IntToString(v.Agirlik)

		// kisibasiAgirlikText := " / Kişi Başı Düşen Tahmini Kilo: " + stnccollection.IntToString(v.Agirlik/v.HissedarAdet)
		...
		//	f.SetCellValue("Sheet1", "A"+stnccollection.IntToString(i), kesimSiraNoText+agirlikTipiText+grupAdiText+kurbanFiyatiText+kupeNoText+KisiBasiDusenHisseFiyatiText+toplamAgirlikText)
		f.SetCellValue("Sheet1", "A"+stnccollection.IntToString(i), kesimSiraNoText+agirlikTipiText+grupAdiText+kupeNoText+toplamAgirlikText)
		...
		for _, kurban := range v.KurbanKisiList {
			f.SetCellStyle("Sheet1", "A"+stnccollection.IntToString(i+2), "H"+stnccollection.IntToString(i+2), styleText1)
			//***** başlar ****---
			...
			f.SetCellValue("Sheet1", "D"+stnccollection.IntToString(i+2), GetGroupToplamOdemeSonucu(kurban.BorcDurum, ac.FormatMoney(kurban.Borc), ac.FormatMoney(kurban.Alacak), ac.FormatMoney(kurban.Bakiye))) //Toplam Ödeme

			f.SetCellValue("Sheet1", "E"+stnccollection.IntToString(i+2), GetGroupKalanBorcSonucu(kurban.BorcDurum, ac.FormatMoney(kurban.Borc), ac.FormatMoney(kurban.Alacak), ac.FormatMoney(kurban.Bakiye))) //Kalan Borç

			f.SetCellValue("Sheet1", "F"+stnccollection.IntToString(i+2), kurban.ReferansAdSoyad+" \n Tel:"+kurban.ReferansTelefon)

			f.SetCellValue("Sheet1", "G"+stnccollection.IntToString(i+2), KisiAciklama(kurban.KisiAciklama))

			f.SetCellValue("Sheet1", "H"+stnccollection.IntToString(i+2), VekaletSonucu(kurban.VekaletDurumu))
...
func KisiAciklama(value string) string {
	if value == "Boş olarak oluşacak grupların buraya kaydedilmesi içindir" {
		return ""
	} else {
		return value
	}
...
func GetGroupToplamOdemeSonucu(borcDurum int, borc string, alacak string, bakiye string) string {
	var ret string = ""
	if borcDurum == 3 {
		ret = borc
...
func GetGroupKalanBorcSonucu(borcDurum int, borc string, alacak string, bakiye string) string {
...
func gruplarBayramilerModel(formType string, c *gin.Context) (grupData entity.SacrificeGruplar, idD uint64, idStr string) {
	id := c.PostForm("ID")
	...
	grupData.Durum, _ = strconv.Atoi(c.PostForm("durum")) //TODO: #BUG bug-stnc buranın html içinde olmamaası gerekiyor
	fmt.Println("ağırlık ")
	...
	if formType == "empty" {
		...
		grupData.Slug = slug
		grupData.Kurban = []entity.SacrificeKurbanlar{}
		for i := 0; i < hissedarAdet; i++ {
			slug = stnchelper.RandSlugV1(2) + stnccollection.IntToString(t.Second()) + stnchelper.RandSlugV1(2)
			a := i
			a++
			appendData := entity.SacrificeKurbanlar{
				KisiID:        1,
				Aciklama:      " ",
				...
				Odemeler: []dto.SacrificeOdemeler{{
