...
func (access *HayvanBilgisi) Edit(c *gin.Context) {
	...
	if kID, err := strconv.ParseUint(c.Param("kID"), 10, 64); err == nil {
		if posts, err := access.HayvanBilgisiApp.GetByID(kID); err == nil {
			...
			access.HayvanBilgisiApp.LastID(kID, &lastID)
			access.HayvanBilgisiApp.NextID(kID, &nextID)
...
/***  DATA MODEL   ***/
func hayvanBilgisiModel(c *gin.Context) (data entity.SacrificeHayvanBilgisi, idD uint64, idStr string) {
	id := c.PostForm("ID")
