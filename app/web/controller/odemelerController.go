...
func InitOdemeler(odemeApp services.OdemelerAppInterface, gkApp services.KurbanAppInterface) *Odemeler {
	return &Odemeler{
...
func (access *Odemeler) Store(c *gin.Context) {
	...
	kalanKurbanFiyati = access.OdemelerApp.OdemelerToplami(KurbanID)
...
func odemelerModel(kurbanSonKalanUcret float64, c *gin.Context) (odemelerData entity.SacrificeOdemeler, idStr string, err error) {
	id := c.PostForm("ID")
	...
	// odemelerData.VerildigiTarih = time.Now()////selBAK
	odemelerData.KurbanFiyati = kurbanFiyati
...
func (access *Odemeler) Makbuz(c *gin.Context) {
	...
	if gID, err := strconv.ParseUint(c.Param("gID"), 10, 64); err == nil {
		if makbuz, err := access.OdemelerApp.GetOdemeRelation(gID); err == nil {
			...
			makbuzOtomatikSiraNo := appOptions.GetOption("makbuz_otomatik_sira_no")
			viewData := pongo2.Context{
				"title":                "Makbuz Kesme",
				"makbuz":               makbuz,
				"csrf":                 csrf.GetToken(c),
