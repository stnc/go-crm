...
func (access *Kisiler) KisiAciklama(c *gin.Context) {
	stncsession.IsLoggedInRedirect(c)

	if ID, err := strconv.ParseUint(c.Param("ID"), 10, 64); err == nil {
		var aciklama string
		access.KisilerApp.GetKisiAciklama(ID, &aciklama)
		// if posts, err := access.KisilerApp.GetKisiAciklama(ID, &aciklama); err == nil {
		// } else {
		// 	c.AbortWithError(http.StatusNotFound, err)
		// }
		viewData := pongo2.Context{
...
//Update data
func (access *Kisiler) KisiAciklamaEdit(c *gin.Context) {
	stncsession.IsLoggedInRedirect(c)
	id := stnccollection.StringtoUint64(c.Query("id"))
	aciklama := c.Query("aciklama")
	access.KisilerApp.SetAciklama(id, aciklama)
	return
	viewData := pongo2.Context{
