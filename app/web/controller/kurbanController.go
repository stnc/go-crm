...
func (access *Kurban) Edit(c *gin.Context) {
	...
	if kurbanID, err := strconv.ParseUint(c.Param("kurbanID"), 10, 64); err == nil {
		if kurbanData, err := access.kurbanApp.GetByID(kurbanID); err == nil {
			...
			db := repository.DB
			appGrup := repository.GruplarRepositoryInit(db)

			grupData, _ := appGrup.GetByID(kurbanData.GrupID)

			var lastID, nextID uint64
			if kurbanData.KurbanTuru == 12 || kurbanData.KurbanTuru == 11 {
				access.kurbanApp.GruopLastID(kurbanID, kurbanData.GrupID, &lastID)
				access.kurbanApp.GruopNextID(kurbanID, kurbanData.GrupID, &nextID)
			} else {
				access.kurbanApp.LastID(kurbanID, &lastID)
				access.kurbanApp.NextID(kurbanID, &nextID)
			}
...
func (access *Kurban) Update(c *gin.Context) {
	...
	if len(savePostError) == 0 {
		...
		if c.PostForm("kaydet") == "gruplaraDon" {
			...
			stncsession.SetFlashMessage("Kayıt başarı ile düzeltildi", "success", c)
...
func (access *Kurban) OdemeEkleCreateModalBox(c *gin.Context) {
	...
	kurbanIDUnit := stnccollection.StringtoUint64(c.DefaultQuery("kurbanID", "0"))

	kurbanOptions := repository.KurbanRepositoryInit(db)

	//TODO: dogru mu calısıyor kontrol et
	kalanKurbanFiyati = kurbanOptions.KalanUcret(kurbanIDUnit)

	// fmt.Println("kalanKurbanFiyati selman")
	...
	viewData := pongo2.Context{
		...
		"csrf":              csrf.GetToken(c),
		"kurbanID":          kurbanID,
		"kalanKurbanFiyati": kalanKurbanFiyati,
	}
	c.HTML(
		http.StatusOK,
...
/***  POST MODEL   ***/
func gkurbanModel(formType string, c *gin.Context) (dataKurban entity.SacrificeKurbanlar, idString string, err error) {
	...
	if formType == "create" {
		...
		dataKurban.BorcDurum = entity.KurbanBorcDurumIlkEklenenFiyat
		dataKurban.Odemeler = []dto.SacrificeOdemeler{{Aciklama: "İlk Eklenen Fiyat", KurbanFiyati: kurbanFiyati, VerilenUcret: 0, Alacak: kurbanFiyati, VerildigiTarih: time.Now(), BorcDurum: entity.OdemelerBorcDurumIlkEklenenFiyat}}
	}
	...
	if formType == "edit" {
		...
		if kurbanFiyati != kurbanFiyatiOLD {
			...
			if kurbanFiyati > odemelerToplami {
				...
				dataKurban.Odemeler = []dto.SacrificeOdemeler{{Aciklama: aciklama, VerilenUcret: 0, KurbanFiyati: kurbanFiyati, Bakiye: odemelerToplami, Alacak: farkUcret, VerildigiTarih: time.Now(), Durum: entity.OdemelerDurumGrupOlusmusKesimlikHayvaniVar, BorcDurum: status}} //TODO: user id eklenecek
			}
			if kurbanFiyati == odemelerToplami {
				//TODO: odemesi 0 ise ödemeler logdurumunda sorun oluyor
				...
				dataKurban.Durum = entity.KurbanDurumGrupOlusmusKesimlikHayvaniVar
				dataKurban.Odemeler = []dto.SacrificeOdemeler{{Aciklama: "Ödeme tamamlandı -fiyat elle değiştirildi", VerilenUcret: 0, Bakiye: odemelerToplami, KurbanFiyati: kurbanFiyati, Alacak: farkUcret, VerildigiTarih: time.Now(), Durum: entity.OdemelerDurumHesapKapandi, BorcDurum: entity.OdemelerBorcDurumFiyatManuelDegistirildiEsitFiyat}} //TODO: user id eklenecek
			}
		}
		...
		if kurbanFiyati == kurbanFiyatiOLD {
			...
			*/
			...
			kalanUcret := Kurban.kurbanApp.KalanUcret(idN)
			kurbanDurum := Kurban.kurbanApp.GetKurbanDurum(idN)
			kurbanBorcDurum := Kurban.kurbanApp.GetKurbanBorcDurum(idN)

			dataKurban.KurbanFiyati = kurbanFiyati
			dataKurban.Alacak = kalanUcret
			dataKurban.Bakiye = odemelerToplami
			dataKurban.BorcDurum = kurbanBorcDurum
			dataKurban.Durum = kurbanDurum
